...
const getPaymentLabel = (method: string, t: any, fallback?: string) => {
  const map: Record<string, string> = {
    cash: "cashOnDelivery",
    card: "creditDebitCard",
    bank_transfer: "bankTransfer",
  };
  return map[method] ? t(map[method]) : fallback || method;
};
...
export default function ProductEditor() {
  ...
  const handleSaveVideo = async () => {
    if (!isEditMode) {
      toast({
        title: t("pleaseSaveProductFirst"),
        variant: "destructive",
      });
      return;
    }
  ...
  // Fetch available badges (unique values from products.badges)
  const { data: availableBadges } = useQuery({
    ...
    queryFn: async () => {
      ...
      for (const row of data || []) {
        const badges = (row as any)?.badges as string[] | null | undefined;
        if (!Array.isArray(badges)) continue;
        for (const b of badges) {
          if (typeof b === "string" && b.trim()) unique.add(b);
        }
      }
  ...
  const { data: availablePaymentMethods } = useQuery({
    ...
    queryFn: async () => {
      ...
      const { data: sellerMethods, error: sellerError } = await supabase
        .from("seller_payment_methods")
        .select("code, name, is_enabled")
        .eq("seller_id", sellerProfile.id)
        .eq("is_enabled", true);

      if (!sellerError && sellerMethods && sellerMethods.length > 0) {
        return sellerMethods
          .filter((m: any) => m?.code)
          .map((m: any) => ({ id: m.code as string, name: m.name as string }));
      }

      const { data: globalMethods, error: globalError } = await supabase
  ...
  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    ...
    try {
      ...
      for (const file of galleryFiles) {
        ...
        const fileName = `gallery_${Date.now()}_${Math.random()
          .toString(36)
  ...
  return (
    <div className="min-h-screen bg-slate-50/50 dark:bg-black">
      ...
      <div className="sticky top-0 z-50 bg-white/80 dark:bg-zinc-950/80 backdrop-blur-xl border-b border-slate-200 dark:border-zinc-800">
        <div className="max-w-full sm:max-w-3xl mx-auto px-4 py-4 flex items-center justify-between">
          ...
          <Button
            ...
            className="rounded-full px-4 sm:px-6 font-black gap-2 shadow-lg shadow-primary/20"
          >
            {isSubmitting ? (
      ...
      <div className="max-w-full sm:max-w-3xl mx-auto px-4 py-8 space-y-8 pb-32">
        <Form {...form}>
          <form
          ...
          >
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl>
                      <Input
                        {...field}
                        className="h-14 rounded-2xl border-slate-200 dark:border-zinc-800 focus:ring-primary text-base font-bold"
                      />
                    </FormControl>
                    <FormMessage />
                  </FormItem>
                      ...
                      <FormControl>
                        <div className="relative">
                          <Input
                            type="number"
                            {...field}
                            className="h-14 rounded-2xl border-slate-200 dark:border-zinc-800 focus:ring-primary text-base font-bold pr-16"
                          />
                  ...
                  render={({ field }) => (
                    <FormItem>
                      ...
                      >
                        ...
                        <SelectContent className="rounded-2xl">
                          <SelectItem value="draft" className="font-bold py-3">
                            {t("draft")}
                          </SelectItem>
                          <SelectItem
                            value="published"
            ...
            <Section
              title={t("logistics")}
              icon={Truck}
              description={t("fulfillmentPromos")}
            >
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                <FormField
                  control={form.control}
                  ...
                  render={({ field }) => (
                    <FormItem className="flex flex-row items-center justify-between rounded-2xl border border-slate-100 dark:border-zinc-900 p-6 bg-slate-50/50 dark:bg-zinc-900/30">
                      <div className="space-y-0.5">
                        <FormLabel className="font-black text-sm uppercase tracking-tight">
                          {t("isDeliverable")}
                        </FormLabel>
                      ...
                      <FormControl>
                        <div className="relative">
                          <Input
                            ...
                            max="100"
                            {...field}
                            className="h-14 rounded-2xl border-slate-200 dark:border-zinc-800 focus:ring-primary text-base font-bold pr-12"
                          />
                          <div className="absolute right-4 top-1/2 -translate-y-1/2 text-sm font-black text-muted-foreground">
                            %
                          </div>
                        </div>
                      </FormControl>
              ...
              <FormField
                ...
                render={() => (
                  <FormItem>
                    <FormLabel className="font-bold text-xs uppercase tracking-widest text-muted-foreground mb-4 block">
                      {t("preferredPaymentMethods")}
                    </FormLabel>
            ...
            <Section
              ...
              icon={Camera}
              description={t("additionalAssets")}
            >
              <div className="space-y-6">
                <div className="grid grid-cols-3 sm:grid-cols-4 gap-4">
                  ...
                  >
                    ...
                    <span className="text-[10px] font-black uppercase text-muted-foreground mt-2 group-hover:text-primary">
                      {t("add")}
                    </span>
                    <input
                      id="gallery-upload"
                ...
                <div className="pt-6 border-t border-slate-100 dark:border-zinc-900">
                  ...
                  <div className="space-y-4">
                    {videos?.map((video: any) => (
                      <div
                        key={video.id}
                        className="relative flex items-center gap-4 p-4 rounded-2xl border border-slate-100 dark:border-zinc-900 bg-white dark:bg-zinc-950 shadow-sm group"
                      >
                    ...
                    {!showAddVideo ? (
                      <Button
                        ...
                        className="w-full h-14 rounded-2xl border-dashed border-2 font-black uppercase tracking-widest text-xs gap-2"
                      >
                        <Plus className="w-4 h-4" />
                        {t("addVideo")}
                      </Button>
                      ...
                      <motion.div
                        ...
                        animate={{ opacity: 1, scale: 1 }}
                        className="border border-slate-200 dark:border-zinc-800 rounded-3xl p-6 space-y-4 bg-slate-50/50 dark:bg-zinc-900/30"
                      >
                        <div className="flex items-center gap-2 p-1 bg-white dark:bg-zinc-950 border border-slate-100 dark:border-zinc-900 rounded-2xl w-fit">
                          <Button
                            type="button"
                            variant={
                              activeVideoTab === "upload" ? "default" : "ghost"
                            }
                            size="sm"
                          ...
                          <Button
                            type="button"
                            variant={
                              activeVideoTab === "url" ? "default" : "ghost"
                            }
                            size="sm"
                        ...
                        {activeVideoTab === "upload" ? (
                          <div
                            className="border-2 border-dashed border-slate-200 dark:border-zinc-800 rounded-2xl p-8 flex flex-col items-center justify-center cursor-pointer bg-white dark:bg-zinc-950 hover:bg-slate-50 dark:hover:bg-zinc-900 transition-all"
                            onClick={() =>
                              document
                                .getElementById("new-video-upload")
                                ?.click()
                            }
                          >
                            {newVideoFile ? (
                        ...
                        <div className="flex gap-2 justify-end">
                          ...
                          <Button
                            type="button"
                            className="rounded-full font-black px-6 gap-2 shadow-lg shadow-primary/20"
                            onClick={handleSaveVideo}
                            disabled={
                              isUploadingVideo ||
                              (!newVideoFile && activeVideoTab === "upload") ||
                              (!newVideoUrl && activeVideoTab === "url")
                            }
                          >
                            {isUploadingVideo ? (
                              <Loader2 className="w-4 h-4 animate-spin" />
                  ...
                  <div className="flex flex-wrap gap-3">
                    <AnimatePresence>
                      {(form.watch("colors") || []).map((color, index) => (
                        ...
                        >
                          <div
                            className="w-5 h-5 rounded-full border border-slate-200 dark:border-zinc-800"
                    ...
                    <div className="relative group/picker">
                      ...
                      <div className="absolute left-1/2 -bottom-6 -translate-x-1/2 opacity-0 group-hover/picker:opacity-100 transition-opacity pointer-events-none">
                        <span className="text-[8px] font-black uppercase text-primary whitespace-nowrap tracking-tighter">
                          {t("addColor")}
                        </span>
                      </div>
                    </div>
            ...
            <div className="p-8 bg-zinc-950 rounded-[40px] text-white overflow-hidden relative group">
              ...
              <div className="relative z-10 space-y-4">
                <h4 className="text-2xl font-black tracking-tighter leading-none italic uppercase">
                  {t("almostReady")}
                </h4>
