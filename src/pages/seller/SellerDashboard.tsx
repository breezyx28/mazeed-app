...
export default function SellerDashboard() {
    ...
    const fetchStats = async () => {
      ...
      try {
        ...
        orderData?.forEach(item => {
          ...
          if (status && !['completed', 'cancelled', 'returned'].includes(status)) {
        ...
        const totalViews = viewsData?.reduce((acc, p) => acc + (p.reviews_count * 12), 0) || 0;
        ...
        if (!rError && reports) {
          reports.forEach(report => {
            actions.push({
              id: `report-${report.id}`,
              type: 'report',
              title: t(report.reported_status === 'wrong_info' ? 'reportWrongInfo' : 
                       report.reported_status === 'out_of_stock' ? 'reportOutofStock' : 
                       report.reported_status === 'discontinued' ? 'reportDiscontinued' : 
                       'reportTemporarilyUnavailable'),
              desc: `${(report.products as any)?.name} (${report.report_count} ${isArabic ? 'بلاغات' : 'reports'})`,
              priority: 'danger',
              icon: AlertTriangle,
        ...
        if (!lsError && lowStockProducts) {
          lowStockProducts.forEach(p => {
            actions.push({
              ...
              desc: `${p.name}: ${p.stock_quantity} ${isArabic ? 'وحدات متبقية' : 'units left'}`,
              priority: 'warning',
              icon: Package,
  ...
  return (
    <div className="min-h-screen bg-slate-50/50 dark:bg-black pb-24">
      {/* Premium Header */}
      <div className="bg-white dark:bg-zinc-950 border-b border-slate-200 dark:border-zinc-800 sticky top-0 z-10">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
             ...
             <div>
                <h1 className="text-xl font-black tracking-tight">{t('sellerDashboard')}</h1>
                <p className="text-[10px] text-muted-foreground uppercase font-bold tracking-widest">{t('manageBusiness')}</p>
             </div>
          </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
              ...
              <div className="space-y-2">
                 <div className="flex items-center gap-2">
                    ...
                    <div className="flex items-center gap-1">
                       ...
                       <span className="text-zinc-500 text-[10px] font-bold uppercase tracking-widest">Premium Partner</span>
                    </div>
                 </div>
                 <h2 className="text-white text-3xl font-black tracking-tighter leading-none">
                    {t('welcomeToStore', { name: sellerProfile?.shop_name || 'Merchant' })}
                 </h2>
             ...
             >
                <div className="flex items-start justify-between mb-4">
                   ...
                   <div className={cn(
                     ...
                     stat.trend === 'up' ? "bg-emerald-500/10 text-emerald-600" : "bg-slate-500/10 text-slate-500"
                   )}>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
           ...
           <div className="lg:col-span-2 bg-white dark:bg-zinc-900 border border-slate-200 dark:border-zinc-800 rounded-[32px] overflow-hidden">
              <div className="p-8 flex items-center justify-between border-b border-slate-100 dark:border-zinc-800">
                 <div className="flex items-center gap-4">
                    ...
                    <div>
                       <h4 className="font-black tracking-tight">{t('storePerformance')}</h4>
                       <p className="text-[10px] text-muted-foreground uppercase font-bold tracking-widest">{t('last30Days')}</p>
                    </div>
                 </div>
                 <Button variant="ghost" size="sm" className="rounded-full text-xs font-bold text-primary">
                    View Insights
                 </Button>
              </div>
              <div className="p-12 text-center h-[240px] flex flex-col items-center justify-center">
                 ...
                 <p className="text-sm text-muted-foreground font-medium max-w-[200px]">Analytics dashboard is being prepared for your shop</p>
              </div>
           </div>
           ...
           <div className="bg-white dark:bg-zinc-900 border border-slate-200 dark:border-zinc-800 rounded-[32px] overflow-hidden flex flex-col">
              <div className="p-8 border-b border-slate-100 dark:border-zinc-800">
                  <h4 className="font-black tracking-tight">{t('pendingAction')}</h4>
                  <p className="text-[10px] text-muted-foreground uppercase font-bold tracking-widest">Needs Attention</p>
              </div>
              <div className="flex-1 p-4 space-y-3">
                 {loading ? (
                     ...
                     >
                        <div className="flex items-center gap-4">
                           <div className={cn(
                             ...
                             action.priority === 'danger' ? "bg-red-100 dark:bg-red-900/40 text-red-600" : "bg-amber-100 dark:bg-amber-900/40 text-amber-600"
                           )}>
                           ...
                           <div className="space-y-1">
                              <div className="text-[13px] font-black tracking-tight flex items-center gap-2 uppercase">
                                {action.title}
                                {action.priority === 'danger' && (
                                  <Badge className="bg-red-600 text-[8px] h-4 px-1 p-0 font-bold uppercase border-none animate-pulse">Critical</Badge>
                                )}
                              </div>
                   ...
                   <div className="flex flex-col items-center justify-center py-12 text-center opacity-40">
                      ...
                      <p className="text-xs font-bold uppercase tracking-widest">{isArabic ? 'كل شيء ممتاز!' : 'All Clear!'}</p>
                      <p className="text-[10px] mt-1 font-medium">{isArabic ? 'لا توجد إجراءات معلقة حالياً' : 'No pending actions at the moment'}</p>
                   </div>
                 )}
              </div>
