    ...
    <div className="min-h-screen bg-slate-50/50 dark:bg-black">
      ...
      <div className="sticky top-0 z-50 bg-white/80 dark:bg-zinc-950/80 backdrop-blur-xl border-b border-slate-200 dark:border-zinc-800">
        <div className="max-w-3xl mx-auto px-4 py-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <div className="flex items-center gap-1">
                ...
                <span className="text-[10px] uppercase font-black text-muted-foreground tracking-widest">Promotion Campaign</span>
              </div>
            </div>
          </div>
          <Button 
          ...
          >
            ...
            {isSubmitting ? (i18n.language === 'ar' ? 'جاري الإنشاء...' : 'Creating...') : t('createOfferBtn')}
          </Button>
        </div>
      </div>
      ...
      <div className="max-w-3xl mx-auto px-4 py-8 space-y-8 pb-32">
        <form id="offer-form" onSubmit={handleSubmit} className="space-y-8">
          ...
          {/* Main Identity */}
          <Section title={t('offerDetails')} icon={Tag} description="CAMPAIGN IDENTITY">
            <div className="space-y-6">
              <div className="space-y-2">
                <Label className="font-bold text-xs uppercase tracking-widest text-muted-foreground">{t('offerName')}</Label>
                <Input 
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="space-y-4">
                  <div className="flex items-center justify-between">
                    <Label className="font-bold text-xs uppercase tracking-widest text-muted-foreground">{t('offerType')}</Label>
                    {!isCreatingType && (
                      <Button 
                        type="button" 
                        variant="ghost" 
                  ...
                  {isCreatingType ? (
                    <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="flex gap-2">
                      <Input 
                        ...
                        className="h-14 rounded-2xl flex-1 font-bold"
                      />
                      <Button type="button" className="h-14 rounded-2xl px-6" onClick={handleCreateType} disabled={isCreatingTypeLoading}>
                        {isCreatingTypeLoading ? <Loader2 className="w-4 h-4 animate-spin" /> : <Plus className="w-4 h-4" />}
                      </Button>
                      <Button type="button" variant="ghost" className="h-14 rounded-2xl px-4" onClick={() => setIsCreatingType(false)}>
                        <X className="w-4 h-4" />
                      </Button>
                    </motion.div>
                  ) : (
                ...
                {/* Expiry Date */}
                <div className="space-y-4">
                  <Label className="font-bold text-xs uppercase tracking-widest text-muted-foreground">{t('offerExpiry')}</Label>
                  <Popover>
                    ...
                    <PopoverContent className="w-auto p-0 rounded-[32px] border-slate-200 dark:border-zinc-800 shadow-2xl overflow-hidden" align="start">
                      <Calendar
                        mode="single"
                        selected={expiryDate}
          ...
          {/* Banner / Poster */}
          <Section title={t('offerPoster')} icon={ImageIcon} description="CAMPAIGN VISUAL">
            <div 
            ...
            >
              {posterPreview ? (
                <img src={posterPreview} alt="Poster" className="absolute inset-0 w-full h-full object-cover transition-transform group-hover:scale-105" />
              ) : (
                <div className="absolute inset-0 flex flex-col items-center justify-center p-8 text-center space-y-4">
                  ...
                  <div>
                    <p className="text-sm font-black uppercase tracking-widest">{t('clickToUploadPoster')}</p>
                    <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest mt-1">Recommended: 1200x500px</p>
                  </div>
                </div>
              )}
          ...
          {/* Product Selection */}
          <Section title={t('selectProducts')} icon={ShoppingBag} description="CAMPAIGN INVENTORY">
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <p className={cn(
                        ...
                        selectedProducts.includes(product.id) ? "text-primary" : "text-black dark:text-white"
                      )}>
                        {product.name}
                      </p>
                      <div className="flex items-center gap-2 mt-1">
                         <span className="text-xs font-bold text-muted-foreground">{product.price} SDG</span>
                         {product.offer_type && (
                ...
                {products?.length === 0 && (
                  <div className="col-span-full py-12 text-center space-y-4">
                     ...
                     <p className="text-sm font-bold text-muted-foreground uppercase tracking-widest">{t('noPublishedProductsFound')}</p>
                  </div>
                )}
              </div>
            </div>
          ...
          <div className="p-10 bg-gradient-to-br from-primary via-primary/90 to-primary/80 rounded-[48px] text-white relative overflow-hidden shadow-2xl group">
             ...
             <div className="relative z-10 space-y-6">
                <div className="space-y-2">
                   <h4 className="text-4xl font-black italic uppercase tracking-tighter leading-tight">Flash Sale or Seasonal?</h4>
                   <p className="text-white/70 font-bold uppercase tracking-widest text-[10px]">Prepare your store for the next big wave of customers.</p>
                </div>
                <Button 
                  type="submit" 
                  disabled={isSubmitting}
                ...
                >
                  ...
                  {isSubmitting ? (i18n.language === 'ar' ? 'جاري الإنشاء...' : 'Launching Campaign...') : t('createOfferBtn')}
                </Button>
             </div>
          </div>
        </form>
